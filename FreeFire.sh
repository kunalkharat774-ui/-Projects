...
.header h1{
  ...
  background:linear-gradient(135deg,#ff6600,#ffaa00,#ff3300);
...
.nav-tabs button{
  background:transparent;border:2px solid #ff6600;color:#ffaa00;
  padding:10px 25px;border-radius:30px;
  font-family:'Orbitron',sans-serif;font-size:0.85em;font-weight:700;
...
.nav-tabs button:hover,.nav-tabs button.active{
  background:#ff6600;color:#000;box-shadow:0 0 20px rgba(255,102,0,0.4);
}
...
.yt-header{
  ...
  border-bottom:1px solid #222;margin-bottom:15px;flex-wrap:wrap;
}
.yt-header .logo{font-family:'Orbitron',sans-serif;font-size:1.8em;font-weight:900;color:#ff0000;}
.yt-header .logo span{color:#fff;}
.yt-search-bar{
  flex:1;min-width:200px;display:flex;gap:0;
}
.yt-search-bar input{
  flex:1;padding:10px 15px;border:2px solid #333;border-right:none;
  border-radius:25px 0 0 25px;background:#111;color:#fff;
  font-size:0.95em;outline:none;
}
.yt-search-bar button{
  padding:10px 20px;border:2px solid #333;border-left:none;
  border-radius:0 25px 25px 0;background:#222;color:#fff;cursor:pointer;
...
.yt-thumb{
  width:100%;height:170px;background:linear-gradient(135deg,#1a1a2e,#16213e,#0f3460);
  display:flex;align-items:center;justify-content:center;
  position:relative;font-size:3em;
}
.yt-thumb .duration{
  position:absolute;bottom:8px;right:8px;background:rgba(0,0,0,0.85);
  padding:3px 8px;border-radius:4px;font-size:0.8em;color:#fff;
}
.yt-info{padding:12px;}
.yt-info h3{font-size:1em;color:#eee;margin-bottom:5px;display:-webkit-box;
...
.yt-watch-page .back-btn{
  background:transparent;border:1px solid #ff6600;color:#ffaa00;
...
.yt-progress{
  height:4px;background:#333;border-radius:2px;margin:8px 0;
  position:relative;overflow:hidden;
}
.yt-progress-bar{
  height:100%;width:0%;background:#ff0000;border-radius:2px;transition:width 0.5s;
}
...
.yt-channel-bar{
  ...
  border-top:1px solid #222;border-bottom:1px solid #222;margin:10px 0;
}
.yt-channel-bar .avatar{
  width:40px;height:40px;border-radius:50%;
  background:linear-gradient(135deg,#ff6600,#ff3300);
  display:flex;align-items:center;justify-content:center;font-weight:900;
}
.yt-channel-bar .sub-btn{
  margin-left:auto;background:#ff0000;border:none;color:#fff;
  padding:8px 18px;border-radius:20px;font-weight:700;cursor:pointer;
}
.yt-comment{
  display:flex;gap:10px;padding:12px 0;border-bottom:1px solid #1a1a1a;
}
.yt-comment .c-avatar{
  width:32px;height:32px;border-radius:50%;
  background:linear-gradient(135deg,#444,#666);flex-shrink:0;
...
.cat-btn{
  ...
  font-weight:600;transition:0.3s;font-size:0.85em;
}
...
.quiz-next-btn{
  ...
  background:linear-gradient(135deg,#ff6600,#ff3300);
  border:none;color:#000;padding:14px 40px;border-radius:30px;
...
.quiz-result{
  ...
  background:#111;border:1px solid #333;border-radius:20px;margin:15px 0;
}
.quiz-result.active{display:block;}
.quiz-result .rank-badge{
  font-size:5em;margin:10px 0;
}
.quiz-result .rank-title{
  font-family:'Orbitron',sans-serif;
  font-size:2em;color:#ff6600;margin:10px 0;
}
...
.restart-btn{
  ...
  font-weight:700;font-size:1em;margin-top:15px;transition:0.3s;
}
...
<div class="container">
  ...
  <div id="section-faketube" class="section active">
    ...
    <div class="yt-watch-page" id="yt-watch-page">
      ...
      <div class="yt-video-player" id="yt-player">
        <div class="play-overlay" onclick="togglePlay()">▶️</div>
      </div>
      ...
      <div style="margin-top:15px;">
        <h3 style="color:#aaa;font-size:1.1em;margin-bottom:10px;">Comments (24)</h3>
        <div id="yt-comments"></div>
      </div>
    </div>
  </div>
...
function animate(){
  ...
  for(let i=0;i<particles.length;i++){
    for(let j=i+1;j<particles.length;j++){
      ...
      if(dist<120){
        ctx.strokeStyle=`rgba(255,102,0,${0.08*(1-dist/120)})`;
...
function renderGrid(videos){
  ...
  videos.forEach(v=>{
    ...
    card.innerHTML=`
      <div class="yt-thumb" style="font-size:3.5em;">${v.emoji}<span class="duration">${v.dur}</span></div>
      <div class="yt-info">
        <h3>${v.title}</h3>
        <p>${v.views} views • ${v.time}</p>
...
function ytSearch(){
  ...
  const filtered=ytVideos.filter(v=>v.title.toLowerCase().includes(q)||v.channel.toLowerCase().includes(q));
  renderGrid(filtered);
}
    ...
    cDiv.innerHTML+=`
      <div class="yt-comment">
        ...
        <div class="c-body">
          ...
          <div class="c-time">2 hours ago</div>
        </div>
      </div>`;
  });
}
...
function renderCategories(){
  const div=document.getElementById('quiz-categories');div.innerHTML=
    `<button class="cat-btn active" onclick="filterCategory('all')">ALL (${allQuestions.length})</button>`;
...
function nextQuestion(){
  ...
  const correct=q.a;
  let isCorrect=false;
  if(sel===correct){isCorrect=true;score++;}
  ...
  filteredQuestions.forEach((q,i)=>{
    ...
    div.innerHTML+=`
      <div class="review-item">
        <div class="r-q">Q${i+1}: ${q.q}</div>
        <div class="r-a ${isCorrect?'r-correct':'r-wrong'}">
          ${isCorrect?'✅ Correct':'❌ Wrong'} — Your answer: ${ans==='N/A'?'Skipped':q.o[ans]}
          ${!isCorrect?` | Correct: ${q.o[q.a]}`:''}
        </div>
      </div>`;
  });
